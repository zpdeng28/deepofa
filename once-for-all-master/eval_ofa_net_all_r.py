...
run_config = ImagenetRunConfig(test_batch_size=args.batch_size, n_worker=args.workers,image_size=[128,160,192,224])
...
# mbv3
#ofa_network.set_active_subnet(ks=7, e=6, d=4)#max
#ofa_network.set_active_subnet(ks=5, e=4, d=3)#mid
#ofa_network.set_active_subnet(ks=3, e=3, d=2)#min
...
# ofa_resnet50
#ofa_network.set_active_subnet(e=0.35, d=2,w=2)#max
#ofa_network.set_active_subnet(e=0.25, d=1,w=1)#mid
ofa_network.set_active_subnet(e=0.2, d=0,w=0)#min
...
img_size,loss_list, acc1_list,acc5_list = run_manager.validate_all_resolution(net=subnet)
for img_size,loss,top1,top5 in zip(img_size,loss_list,acc1_list,acc5_list):
    print('Results: size=%d,\t loss=%.5f,\t top1=%.1f,\t top5=%.1f' % (img_size,loss, top1, top5))
